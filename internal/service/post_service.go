...
func (s *PostService) ListVideos(ctx context.Context, viewerID *uuid.UUID, cursor *time.Time, limit int) (*domain.FeedResult, error) {
	if limit <= 0 || limit > 50 {
		limit = 21
	}

	posts, err := s.repo.ListVideos(ctx, viewerID, cursor, limit)
	if err != nil {
		return nil, err
	}
